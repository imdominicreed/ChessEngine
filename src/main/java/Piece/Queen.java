...
public class Queen extends Piece{
    ...
    @Override
    public Piece clone() {
        Queen holder = new Queen(PieceCoordinate, color, pieceMoved);
        return holder;
    }
    ...
    public static boolean EigthColumnExceptions(int coord, int direction){
        if(Utilities.isEigthColumn(coord) && (direction == 7 || direction == -9|| direction == -1)) {
            return true;
        }
        return false;
    }
    public static boolean FirstColumnExceptions(int coord, int direction) {
        if (Utilities.isFirstColumn(coord) &&  (direction ==-7 || direction == 9 || direction == 1)) {
            return true;
        } else {
            return false;
        }
